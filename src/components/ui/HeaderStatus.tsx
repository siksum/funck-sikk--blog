  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-800/50">
      ...
      <div className="flex items-center gap-1 text-xs whitespace-nowrap">
        <span className="text-gray-700 dark:text-zinc-200">주인장은 지금</span>
        {status?.app ? (
          <>
            <span className="text-violet-600 dark:text-violet-400 font-medium">
              {status.app}
            </span>
            <span className="text-gray-700 dark:text-zinc-200">하는 중</span>
          </>
        ) : (
          <span className="text-gray-700 dark:text-zinc-200">자리 비움 💤</span>
        )}
      </div>
    </div>
  );
