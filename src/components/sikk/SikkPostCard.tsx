...
export default function SikkPostCard({ post, variant = 'default' }: SikkPostCardProps) {
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        <h3
          className="text-sm font-medium group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors line-clamp-2"
    ...
    return (
      <article
        className="group rounded-xl overflow-hidden transition-all duration-300 backdrop-blur-xl
          border border-gray-200 dark:border-pink-500/30
          hover:border-pink-300 dark:hover:border-pink-400/60
      ...
      >
        <Link href={`/sikk/${post.slug}`} className="block">
          <div className="p-4">
            ...
            <div className="flex items-center gap-1 mb-2 flex-wrap">
              {post.categoryPath.map((name, index) => (
                <span key={index} className="flex items-center">
                  ...
                  <span
                    className="px-1.5 py-0.5 text-xs font-medium rounded border border-pink-200 dark:border-pink-500/40
            ...
            {/* Title */}
            <h2
              className="text-base font-semibold group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors mb-1 line-clamp-1"
              style={{ color: 'var(--foreground)' }}
            >
              {post.title}
            </h2>
            ...
            <div className="flex items-center justify-between text-xs" style={{ color: 'var(--foreground-muted)' }}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={handleShare}
                  className="p-1 hover:text-pink-600 dark:hover:text-pink-400 transition-colors"
                  aria-label="공유하기"
                >
    ...
    >
      <Link href={`/sikk/${post.slug}`} className="block">
        ...
        {post.thumbnail ? (
          ...
          <div
            className="relative w-full h-36 overflow-hidden flex items-center justify-center bg-gradient-to-br from-pink-100 to-rose-100 dark:from-pink-900/30 dark:to-rose-900/30"
          >
        ...
        <div className="p-4">
          ...
          {/* Title */}
          <h2
            className="text-base font-semibold group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors mb-1 line-clamp-2"
            style={{ color: 'var(--foreground)' }}
          >
            {post.title}
          </h2>
          ...
          <div className="flex flex-col gap-1.5 text-xs" style={{ color: 'var(--foreground-muted)' }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {post.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="px-1.5 py-0.5 rounded border border-pink-200 dark:border-pink-500/40
                    bg-pink-50 dark:bg-zinc-800 text-gray-700 dark:text-zinc-200"
                >
                  #{tag}
                </span>
