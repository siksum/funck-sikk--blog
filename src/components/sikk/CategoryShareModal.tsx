...
export default function CategoryShareModal({
  slugPath,
  categoryName,
  isOpen,
  onClose,
  ...
  const slugPathParam = slugPath.join('/');

  const fetchShareSettings = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/sikk/categories/share?slugPath=${encodeURIComponent(slugPathParam)}`);
      if (res.ok) {
        ...
        setIncludeSubcategories(data.share?.includeSubcategories ?? true);
      }
    } catch (error) {
  ...
  const handleTogglePublic = async () => {
    try {
      setSaving(true);
      const res = await fetch(`/api/sikk/categories/share`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          slugPath: slugPathParam,
  ...
  const handleToggleIncludeSubcategories = async () => {
    try {
      setSaving(true);
      const newValue = !includeSubcategories;
      const res = await fetch(`/api/sikk/categories/share`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (res.ok) {
        const data = await res.json();
        setShare(data.share);
        setIncludeSubcategories(newValue);
      }
    } catch (error) {
      console.error('Failed to toggle include subcategories:', error);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      const emails = inviteEmail
        ...
        .map((e) => e.trim())
        .filter((e) => e);
      const res = await fetch(`/api/sikk/categories/share/invitations`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          slugPath: slugPathParam,
  ...
  const handleRemoveInvitation = async (email: string) => {
    try {
      ...
      const res = await fetch(
        `/api/sikk/categories/share/invitations?slugPath=${encodeURIComponent(slugPathParam)}&email=${encodeURIComponent(email)}`,
        {
          method: 'DELETE',
        }
      );
  ...
  const getShareUrl = () => {
    ...
    return `${window.location.origin}/sc/${share?.publicToken}`;
  };
      ...
      <div className="relative w-full max-w-lg mx-4 bg-white dark:bg-gray-800 rounded-xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-800 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">카테고리 공유</h2>
            <button
              onClick={onClose}
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400 truncate">{categoryName}</p>
        </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">하위 카테고리 포함</p>
              ...
              <button
                onClick={handleToggleIncludeSubcategories}
                disabled={saving}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  includeSubcategories ? 'bg-pink-500' : 'bg-gray-300 dark:bg-gray-600'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    includeSubcategories ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
